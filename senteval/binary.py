...
'''
Binary classifier and corresponding datasets : MC, FAC
'''
...
class MCEval(BinaryClassifierEval):
    def __init__(self, task_path, seed=1111):
        logging.debug('***** Transfer task : MC *****\n\n')
        pos = self.loadFile(os.path.join(task_path, 'muchocine.pos'))
        neg = self.loadFile(os.path.join(task_path, 'muchocine.neg'))
        super(self.__class__, self).__init__(pos, neg, seed)


class FACEval(BinaryClassifierEval):
    def __init__(self, task_path, seed=1111):
        logging.debug('***** Transfer task : FAC *****\n\n')
        obj = self.loadFile(os.path.join(task_path, 'fac.objective'))
        subj = self.loadFile(os.path.join(task_path, 'fac.subjective'))
